...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const entriesRes = await fetch(
      `https://api.clockify.me/api/v1/workspaces/${workspaceId}/user/${targetUser.id}/time-entries?start=${formatDateForClockify(from!, false)}&end=${formatDateForClockify(to!, true)}&page-size=1000`,
      { headers: { "X-Api-Key": apiKey } }
    )
